...
def save_checkpoint(result_dir, epoch, step, model, optimizer, lr_scheduler):
  checkpoint_dir = get_checkpoint_dir(result_dir)
...
def load_checkpoint(result_dir, device, epoch=None, model=None, optimizer=None, lr_scheduler=None):
  ...
  checkpoint = torch.load(checkpoint_filename, map_location={"cuda:0": "cpu"})
