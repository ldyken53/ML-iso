...
def parse_args(cmd=None, description=None):
  ...
  if cmd in {'preprocess', 'infer', 'infer_temporal'}:
  ...
  if cmd in {'train', 'train_temporal', 'train_deepfovea', 'train_temporal_opticalflow', 'train_temporal_gan', 'find_lr', 'infer', 'infer_temporal', 'export', 'visualize'}:
  ...
  if cmd in {'train', 'train_temporal', 'train_deepfovea', 'train_temporal_opticalflow', 'train_temporal_gan', 'infer', 'infer_temporal', 'export'}:
    parser.add_argument('--checkpoint', '-c', type=int, default=0, help='result checkpoint to restore')

  if cmd in {'train', 'train_temporal', 'train_deepfovea', 'train_temporal_opticalflow', 'train_temporal_gan', 'infer', 'infer_temporal', 'find_lr', 'vgg19_helper'}:
  ...
  if cmd in {'export'}:
    ...
    parser.add_argument('--input_dimensions', type=int, nargs='*', default=[1, 3, 256, 256], help='input dimensions to trace the model with')

  if cmd in {'train_temporal', 'train_deepfovea', 'train_temporal_opticalflow', 'train_temporal_gan', 'infer_temporal'}: 
    parser.add_argument('--temp_size', type=int, default=16, help='Number of frames to use for temporal training and inference')


  if cmd in {'convert_image', 'convert_dataset', 'temporalize_dataset'}:
  ...
  if cmd in {'preprocess', 'train', 'train_temporal', 'train_deepfovea', 'train_temporal_opticalflow', 'train_temporal_gan', 'infer', 'infer_temporal', 'export', 'temporalize_dataset'}:
