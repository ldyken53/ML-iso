  ...
  with torch.no_grad():
    for group, input_names, target_name in image_sample_groups:
      ...
      for input_name in input_names:
        ...
        #masks = GetSampleMaskSequence((input.shape[-2], input.shape[-1]), input.shape[-2]//5, num=cfg.temp_size, threshold=0.95, mask_size=256, noise_type='uniform')
        masks = GetSampleMaskSequence((input.shape[-2], input.shape[-1]), input.shape[-2]//5, num=cfg.temp_size, threshold=0.99, mask_size=128, noise_type='stbn')
        for t in range(cfg.temp_size):
          idx = (masks[t] == 0)
          input[:, t, :, idx] = 0
        ...
        if cfg.report is not None:
          for t in range(cfg.temp_size):
            if t > 0:
              row = generate_report_row(t, output[:, t, ...], target[:, t, ...], output[:, t-1, ...], target[:, t-1, ...])
            else:
              row = generate_report_row(t, output[:, t, ...], target[:, t, ...])
            report.append(row)
      ...
      if cfg.save_all and target_name and not cfg.report:
