...
def main_worker(rank, cfg):
  ...
  for epoch in range(start_epoch, cfg.epochs+1):
    ...
    for i, batch in enumerate(train_loader, 0):
      ...
      loss = 0.8 * spatial_loss + 1.0 * temporal_loss + 0.1 * of_loss
      loss.backward()
    ...
    if rank == 0:
      ...
      progress.finish('loss=%.6f, spatial_loss=%.6f temp_loss=%.6f of_loss=%.6f lr=%.6f  (%.1f images/s, %s, eta %s)'
                      % (train_loss, train_spatial_loss, train_temp_loss, train_of_loss, lr, images_per_sec, format_time(duration), format_time(eta, precision=2)))
