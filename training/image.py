...
def image_sequence_to_tensor(images, batch=False):
  # Reorder from HWC to CHW
  tensor = torch.from_numpy(images.transpose((0, 3, 1, 2)))
  if batch:
...
# Compares two image tensors using the specified error metric
def compare_image_sequences(a, b, metric='psnr'):

  if a.size()[1] != b.size()[1]:
    raise ValueError('temporal size mismatch')

  if metric == 'mse':
    return torch.mean(torch.FloatTensor([((a[:,t,...] - b[:,t,...]) ** 2).mean() for t in range(a.size()[1])]))
  elif metric == 'psnr':
    mse = torch.mean(torch.FloatTensor([((a[:,t,...] - b[:,t,...]) ** 2).mean() for t in range(a.size()[1])]))
    return 10 * np.log10(1. / mse.item())
  elif metric == 'ssim':
    return torch.mean(torch.FloatTensor([ssim(a[:,t,...], b[:,t,...], data_range=1.) for t in range(a.size()[1])]))
  else:
    raise ValueError('invalid error metric')
