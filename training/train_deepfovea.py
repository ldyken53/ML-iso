  ...
  for epoch in range(start_epoch, cfg.epochs+1):
    ...
    for i, batch in enumerate(train_loader, 0):
      ...
      for t in range(cfg.temp_size):
        ...
        spatial_loss +=  criterion_spatial(outputs[-1], target[:, t, ...])
        if t > 0:
          # for dt in range(0,t):
          ...
          I, motion = reproject(target[:, t-1, ...], target[:, t, ...], kernel_size = 15, return_motion = True)
          #error_blend = get_error_norm_blend_factor(target[:, t, ...], I, a=1.0)
          # if rank == 0 and i%20==0 and t == cfg.temp_size-1:
          #   save_image('proj_out.png', tensor_to_image((error_blend * I)[0, 0:3, ...].detach()))
          #   save_image('proj_src.png', tensor_to_image(target[0, t-1, ...].detach()))
          #   save_image('proj_dest.png', tensor_to_image(target[0, t, ...].detach()))
          
          of_loss += criterion_temporal(outputs[-1], fun.grid_sample(outputs[-2], motion, align_corners=False))
      ...
      loss = 1.0 * spatial_loss + 0.1 * of_loss
      loss.backward()
    ...
    if distributed:
      ...
      dist.all_reduce(train_of_loss, op=dist.ReduceOp.SUM)
    ...
    train_of_loss = train_of_loss.item() / (train_steps_per_epoch * cfg.num_devices)
    ...
    if rank == 0:
      ...
      summary_writer.add_scalar('opticalflow_loss', train_of_loss, epoch)
    ...
    if rank == 0:
      ...
      progress.finish('loss=%.6f, spatial_loss=%.6f of_loss=%.6f lr=%.6f  (%.1f images/s, %s, eta %s)'
                      % (train_loss, train_spatial_loss, train_of_loss, lr, images_per_sec, format_time(duration), format_time(eta, precision=2)))
